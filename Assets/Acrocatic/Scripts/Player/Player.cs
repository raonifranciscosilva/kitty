...
namespace Acrocatic {
	...
	public class Player : MonoBehaviour {
		...
		// Public variables.
		[Tooltip("Selecione a direção do sprite.")]
		public Direction spriteDirection;
		[Tooltip("Selecione a layer para as plataformas.")]
		public LayerMask platformLayer;
		[Tooltip("Selecione a layer para o chão.")]
		...
		void FixedUpdate() {
            ...
            if (!dashing && !wallJumping && !wallRunning && !wallSliding && !falling && !onLadder && !crouching)
            {
                if (Input.GetButton("Atirar") && proximoLancar <= Time.time) //&& !dashing && !wallJumping && !wallRunning && !wallSliding && !falling && !onLadder)
                {
                    Atira();
                    animator.SetTrigger("startLancar");
                    animator.SetTrigger("lancando");
                    lancando = false;
		...
		void Update() {
            ...
            animator.SetBool("lancando", lancando); //Ataque
		...
		public void Dead() {
			...
			SetJumps(0);
			falling = true;
            animator.SetTrigger("startMorte");
        ...
        public void Lancar(bool lancar)
        {
            // Call the trigger 'stateComplete' when dashing is finished.
            if (lancando && !lancando)
            {
                animator.SetTrigger("stateComplete");
                // Start the dash when dashing.
            }
            else if (lancar && !lancando)
            {
                animator.SetTrigger("startLancar");
            }
            lancando = lancar;
